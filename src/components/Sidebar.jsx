    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${ selectedUser ? "max-md:hidden" : "" }`}>
      ...
      <div className='pb-5'>
        ...
        <div className='flex justify-between items-center'>
          ...
          <img src={assets.logo} alt="logo_img" className='max-w-40' />

          <div className='relative py-2 group'>
            <img src={assets.menu_icon} alt="menu_icon" className='max-h-5 cursor-pointer'/>
            <div className='absolute hidden group-hover:block top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100'>
      ...
      <div className='flex flex-col'>
        {
          filteredUsers.map(( user, index )=>(
            <div onClick={()=> setSelectedUser(user)} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50' }`}>
              <img src={user?.profilePic || assets.avatar_icon} alt="profilePic" className='w-8.75 aspect-square rounded-full' />
